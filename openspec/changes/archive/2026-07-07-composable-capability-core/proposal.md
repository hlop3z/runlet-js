...
- **Capability-owned type surface (D11)**: each `CapabilityDef` carries its own `.d.ts` fragment beside its JS wrapper; the single `container/types.d.ts` is machine-assembled from a core base fragment + each registered capability's fragment and golden-tested for drift, instead of being a hand-maintained monolith a third-party author cannot compose against.
- **BREAKING (D12)**: the script-facing HTTP global is renamed `api` → `http`, aligning the last vestigial `api` label with the already-`http` internals (module/hook/feature) and the resource-named convention of every other global. Method names and the `allowed_hosts` gating key are unchanged; rides the same pre-publish clean-break window.
