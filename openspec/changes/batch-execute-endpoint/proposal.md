...
- New `POST /execute/batch` endpoint: `{ items: [ {script|key, context, config}, … ] }` →
  `{ results: [ {data,error,meta}, … ], meta: {items, ok, failed, duration_ms, trace_id} }`,
  order-preserving.
...
- Batch-level caps enforced before any admission: `max_batch_items` and a combined-bytes
  limit; oversize batches rejected whole with a request-category error.
...
- **Code**: `runlet/src/handler.rs` (+ router in `main.rs`), config (`max_batch_items`,
  combined-bytes cap), events/quota touch only in that they are invoked per item.
