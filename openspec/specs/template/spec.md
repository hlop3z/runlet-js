...
The `template` capability is the always-on, pure string-templating value-util for the sandbox:
`$std.template` turns a JSON context into a human-facing string (invoices, HTML/plain email, SMS,
receipts) using Jinja2 syntax (`{{ expr }}` / `{% stmt %}`). It exposes two explicit escaping modes —
`html` (auto-escaping, for browser/HTML-email output) and `text` (verbatim, for plain email/SMS) —
with no ambiguous default, so the author always states the output kind. A compiled template renders a
context with `.render`, tolerates missing merge tags leniently (empty by default, or a settable
`.missing` placeholder), and reports its referenced merge tags via `.fields`. It is pure (no I/O, no
clock, no randomness) and injected identically under every profile. Rationale: `src/template.rs`,
`src/js/template.js`, `docs/14-template.md`, and the ADR in
`openspec/changes/std-template-util/design.md`.
