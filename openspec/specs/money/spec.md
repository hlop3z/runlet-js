...
The `$` / `money` global gives handlers a **currency-bound money value** that is safe by
construction inside the QuickJS sandbox: arithmetic only combines the same currency (no implicit
FX), precision follows the currency's ISO 4217 minor unit, and splitting is penny-exact. It is the
headline value-util of a business-scripting sandbox — the `$` glyph carries the money abstraction —
and composes over the exact-number `Decimal` core (see the `decimal` capability). Every
author-facing method is snake_case. Rationale: `src/money.rs`, `src/js/money.js`,
`docs/05-decimal.md`, and the `money-value-util` change design.
