...
in-band), and meters every call into `meta.io.http`. Source of truth: `src/http.rs`,
`src/js/http.js`, `docs/02-api.md`.

## Requirements

### Requirement: Opt-in injection via allowed_hosts

The `http` global SHALL be injected only when the request `config.allowed_hosts` is present and
non-empty; otherwise the `http` global SHALL NOT exist in the handler scope.

#### Scenario: Capability absent without config

- **WHEN** a request omits `config.allowed_hosts` or supplies an empty list
- **THEN** the handler observes `typeof http === "undefined"`
...
The `http` global SHALL expose `get(url, params, headers)`, `post(url, body, headers)`,
...
Each `http` call SHALL count against the per-execution `max_ops` budget, and a call made after
the budget is exhausted SHALL fail.

#### Scenario: Op budget exhausted

- **WHEN** a handler issues an `http` call after `max_ops` external operations are already used
- **THEN** the call returns an in-band error with code `HTTP_OP_LIMIT`

### Requirement: Per-call metering

Every `http` call SHALL be recorded and surfaced in `meta.io.http`, including blocked and
failed calls, without leaking the request path or query.

#### Scenario: Successful call metered

- **WHEN** a handler makes an `http` call
- **THEN** `meta.io.http` gains an entry with method, host, status, request/response byte sizes, and duration

#### Scenario: Blocked call still metered

- **WHEN** an `http` call is blocked or fails (status reported as 0)
- **THEN** it still appears in `meta.io.http` with `status` 0, and only the host (no path or query) is recorded
