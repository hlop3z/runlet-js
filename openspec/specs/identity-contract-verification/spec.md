# identity-contract-verification Specification

## Purpose

The opt-in `trusted.contract` sub-mode: behind current nexus (which retired the bare
`x-user-roles`/`x-user-entitlements`/`x-user-suspended` headers into a signed `x-identity-contract`),
the box verifies that ES256 JWS and sources the revocation-sensitive identity from its claims. This
capability owns the verification surface — JWKS acquisition/caching/rotation, signature and
registered-claim checks, the `ctr` version drift gate, and fail-closed behavior — while
`tenant-identity` owns how the verified claims feed the request gates. Off by default; the plain
trusted-header path is unchanged when it is off. Rationale: `docs/design/multitenant-trust.md` and the
change's `design.md` (D6/D7).
