# tenant-audit Specification

## Purpose

Per-request, per-tenant decision events — the compliance trail. For every trusted-mode request the
box emits one `audit` event recording the terminal decision: `allowed` when the request ran, or
`denied` with a machine-readable reason code when a gate terminated it (anonymous / suspended /
tenant-less / non-acting scope / missing entitlement / quota / oversized / egress / overload). Audit
events share the versioned envelope of `tenant-metering`, are keyed by tenant + user, and replace the
blind aggregate rejection counter with attributed, reasoned records. Identity is carried only for
attribution — no request bodies, secrets, or credentials. See `docs/deployment.md`.
