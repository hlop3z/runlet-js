...
- Egress port: Rust **`Egress`** (`runlet_wire::Egress`/`EgressError`); script-facing global +
...
Six workspace members (`Cargo.toml`): `runlet-wire`, `fabric-backends`, `fabricd`, `runlet-core`,
`runlet`, `runlet-bench`.
- **`runlet-wire`** — shared leaf, no drivers/QuickJS: `Egress`+`EgressError`, the taxonomy
...
- **`fabric-backends`** — the driver bag (featureless, all 6): `*Backend`s + `*Config` +
  ...
  Depends on `runlet-wire` ONLY. **Only `fabricd` links it.**
- **`runlet-core`** — the sandbox. Links NO driver even with `full`. Slim JS-wrapper modules
  ...
  `runlet-wire`. `http`/`s3` in-engine. `LogicHost::new(pool, registry, settings)` — handle/breaker
  gone.
...
- **Step 5 (full trust flip):** creds live ONLY in `fabricd`. The `ResourceBinding` table +
  ...
  moved `fabric-backends`→`runlet-wire`. **No in-process fallback:** driver request + no/unreachable
...
- **Verified (Docker `rust:1.92-alpine`):** full `cargo clippy` clean; per-cap cfg sweep clean
  (`sweep.sh`; needed `tokio` `io-util` on `runlet-wire` so the deterministic-core build has the
  framing traits); `cargo test` green (runlet-wire 8, fabric-backends 17, runlet-core 40, runlet 9);
...
- `fabric-backends` must NOT depend on `runlet-core` (would pull QuickJS into `fabricd`). Anything
  the backends share with the sandbox (wire types, breaker, collector) lives in `runlet-wire`.
...
- `unused_crate_dependencies` is denied → the binary reaches wire types via `runlet_core`
  re-exports (no direct `runlet-wire` dep); `fabric-backends` gates every driver dep is moot since
