//! The per-target circuit breaker (Tier 3), re-exported from [`runlet_wire`].
//!
//! The breaker moved to the shared `runlet-wire` crate so the driver host (`fabric-backends`)
...
pub use runlet_wire::breaker::{BreakerConfig, CircuitBreaker};
