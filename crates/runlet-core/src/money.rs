//! Currency-bound money helper for the `QuickJS` sandbox (`$std.money`, mirrored to the `$` global).
//!
//! `$(amount, currency?)` builds a currency-bound value that is safe by construction (same-currency
//! arithmetic, no implicit FX, per-currency minor units). It is pure JS composed over the
//! `__decimal` FFI and the ISO 4217 exponent table in `money.js` — no Rust FFI of its own — so it is
//! always injected, right after `$std.decimal`.
//!
